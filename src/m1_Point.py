...
class Point(object):
    ...
    def get_distance_from_start(self):
        import math
        start_dist = math.sqrt((self.x - self.startx)**2 + (self.y - self.starty)**2)
        return start_dist
    ...
    def closer_to(self, p2, p3):
        import math
        dist2 = math.sqrt((self.x - p2.x)**2 + (self.y - p2.y)**2)
        dist3 = math.sqrt((self.x - p3.x)**2 + (self.y - p3.y)**2)
        if dist2 <= dist3:
            return p2
        else:
            return p3
    ...
    def halfway_to(self, p2):
        self.halfx = (self.x + p2.x)/2
        self.halfy = (self.y + p2.y)/2
        return Point(self.halfx, self.halfy)
